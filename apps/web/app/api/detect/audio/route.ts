...
function runPythonInference(args: string[], cwd: string): Promise<PythonResult> {
  const pythonBin = process.env.AUDIO_PYTHON_PATH ?? "python";
...
export async function POST(request: Request) {
  ...
  const modelPath = path.join(repoRoot, "models", "audio", "bird_classifier_full.pth");
  ...
  const extension = path.extname(uploaded.name || "").toLowerCase() || ".wav";
  const tempPath = path.join(tmpdir(), `birdlens-audio-${randomUUID()}${extension}`);
